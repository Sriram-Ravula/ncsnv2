...
def run_vol(args_score, config_score, args_par, config_par):
    ...
    if args_score.rank == 0:
        ...
        runs_per_sample = args_par.num_samples_per_slice
        ...
        results_psnr = torch.zeros(runs_per_sample, num_samples, num_outs, device=config_score.device)
        results_ssim = torch.zeros(runs_per_sample, num_samples, num_outs, device=config_score.device)
        results_mse = torch.zeros(runs_per_sample, num_samples, num_outs, device=config_score.device)

        if args_par.save_all_intermediate:
            output_shape = [runs_per_sample, num_samples, num_outs, 1, output_shape['ld'][1], output_shape['ld'][0]]
        else:
            output_shape = [runs_per_sample, num_samples, 1, output_shape['ld'][1], output_shape['ld'][0]]
        results_out = torch.zeros(output_shape, device=config_score.device)

    for run_num in range(args_par.num_samples_per_slice):
        for i, batch in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                for l in args_par.levels:
                    ...
                    for t in range(args_par.tmax):
                        ...
                        if args_par.eta_noise > 0:
                            noise = torch.randn_like(x_mod) * args_par.eta_noise 
                        else:
                            noise = torch.zeros_like(x_mod)
                        
                        x_mod = x_mod + step_size*base_grad + torch.sqrt(step_size * 2)*noise
            ...
            if args_score.rank == 0:
                ...
                for i in range(len(out_samples)):
                    ...
                    if args_par.save_all_intermediate:
                        results_out[run_num, idx] = out_samples[i].detach().clone()
                    else:
                        results_out[run_num, idx] = out_samples[i].detach().clone().squeeze(0)

                    results_psnr[run_num, idx] = out_psnr[i].detach().clone()
                    results_ssim[run_num, idx] = out_ssim[i].detach().clone()
                    results_mse[run_num, idx] = out_mse[i].detach().clone()
