...
@torch.no_grad()
def anneal_Langevin_dynamics_rtm_n(x_mod, scorenet, lambdas, n_shots, n_steps_each=200, step_lr=0.000008,
                             final_only=False, verbose=False, denoise=True, add_noise=True):
    images = []

    with torch.no_grad():
        for c, sigma in enumerate(lambdas):
            labels = torch.ones(x_mod.shape[0], device=x_mod.device) * c #dummy target 1...T depending on iteration
            labels = labels.long()
            step_size = step_lr * (sigma / lambdas[-1]) ** 2
        ...
        if denoise:
            last_noise = (len(lambdas) - 1) * torch.ones(x_mod.shape[0], device=x_mod.device)
            last_noise = last_noise.long()
            x_mod = x_mod + lambdas[-1] ** 2 * scorenet(x_mod, last_noise)
            images.append(x_mod.to('cpu'))
